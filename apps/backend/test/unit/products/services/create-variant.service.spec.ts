  ...
  describe('addVariant', () => {
    ...
    it('Debería añadir una variante exitosamente', async () => {
      ...
      mockProductUtils.extractVariantDetails.mockReturnValue(
        validDto.notebookVariantDetails,
      );
      ...
      expect(productUtils.generateSku).toHaveBeenCalledWith(
        'NOTEBOOK',
        'Mi Producto',
        validDto.notebookVariantDetails,
      );
    ...
    it('Debería lanzar NotFoundException si el producto no existe', async () => {
      ...
      await expect(service.addVariant('no-existe', validDto)).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it('Debería lanzar BadRequestException si faltan detalles específicos del tipo', async () => {
      ...
      await expect(service.addVariant(slug, validDto)).rejects.toThrow(
        BadRequestException,
      );
    });
